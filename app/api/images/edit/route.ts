...
import { editGeminiImage } from "@/lib/litellm-client";

export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { model, prompt, images, aspectRatio, imageSize, numImages = 1 } = body;
    ...
    if (!model || !prompt || !images || !Array.isArray(images) || images.length === 0) {
      return NextResponse.json(
        { error: "Missing required fields: model, prompt, and images (array)" },
        { status: 400 }
      );
    }
    ...
    // Call Gemini API through LiteLLM proxy with image input
